...
export default function Navbar({ siteName }: { siteName: string }) {
  ...
  const navbarVariants: Variants = {
    hidden: { y: isMobile ? 50 : -50, opacity: 0 },
    show: {
      y: 0,
      opacity: 1,
      transition: {
  ...
  return (
    <div className="relative w-full">
      ...
      {/* Mobile Bottom Scroll Blur Mask */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: isScrolled ? 1 : 0 }}
        transition={{ duration: 0.5, ease: smoothEase }}
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.header
            ...
            animate="show"
            exit="hidden"
            className="fixed inset-x-0 bottom-3 z-[100] flex justify-center px-3 max-lg:bottom-[max(0.75rem,env(safe-area-inset-bottom))] lg:top-4 lg:bottom-auto"
          >
            <Wrapper
              className={cn(
