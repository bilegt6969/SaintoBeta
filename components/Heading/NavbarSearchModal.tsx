...
import type { SearchProduct } from "lib/commerce/types";
import { Clock, Reply, Search, X } from "lucide-react";
...
export function NavbarSearchModal({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      fetch(`/api/search?q=${encodeURIComponent(q)}`, {
        ...
        .then((res) => res.json())
        .then((json: { products?: SearchProduct[] }) => {
          if (Array.isArray(json?.products)) {
            setSearchProducts(json.products.slice(0, 4));
          } else {
            setSearchProducts([]);
          }
        })
  ...
  const contentVariants: Variants = {
    ...
    show: {
      opacity: 1,
      y: 0,
      transition: { delay: 0.15, duration: 0.55, ease: easeOutSmooth },
    },
    exit: {
      opacity: 0,
      y: -6,
  ...
  const itemVariants: Variants = {
    ...
    exit: {
      opacity: 0,
      x: -8,
      transition: { duration: 0.35, ease: easeInOutSmooth },
    },
  };
  ...
  const renderSearchInput = () => (
    <form onSubmit={handleSubmit} className="relative group">
        ...
        <AnimatePresence mode="wait">
          {!hasTyped ? (
            <motion.div
              ...
              className="shrink-0"
            >
              <Search className="w-4 h-4 text-white/60" strokeWidth={2} />
            </motion.div>
          ) : (
            <motion.span
              key="showme"
      ...
      <div className="relative">
        <BorderBeam size="md" colorVariant="mono" className="rounded-full">
          <input
            ref={inputRef}
            type="text"
            value={searchQuery}
        ...
        <motion.button
          ...
          animate={{
            ...
            scale: searchQuery.trim().length > 0 ? 1 : 0.8,
            x: searchQuery.trim().length > 0 ? 0 : 10,
          }}
          transition={{
            duration: 0.3,
            ease: [0.22, 1, 0.36, 1],
          ...
          className="absolute right-14 top-1/2 -translate-y-1/2 flex items-center justify-center transition-colors z-20 hover:opacity-70"
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
      {searchProducts.map((product) => (
        ...
        >
          ...
          <div className="p-2">
            <p className="text-[12px] text-white/90 line-clamp-2 min-h-[2.25rem]">
              {product.title}
            </p>
            {formatPriceUsd(product) !== "N/A" && (
              <p className="text-[11px] text-white/50 mt-0.5">
                {formatPriceUsd(product)}
              </p>
            )}
          </div>
        </Link>
          ...
          <div className="hidden md:block fixed inset-0 z-[110] pointer-events-none overflow-visible">
            <div className="flex items-start justify-center pt-32 px-4 h-full pointer-events-none overflow-visible">
              ...
              >
                ...
                <div className="relative">{renderSearchInput()}</div>
                <AnimatePresence mode="wait">
